...
int l = message.Length / (key1.Length * key2.Length);
if (message.Length % (key1.Length * key2.Length) > 0) l++;
 ...
 return res;
   ...
   public static string MessToSubstringsDe(string message, string key1, string key2)
   {
       ...
       string[] matrix = new string[(message.Length/(key1.Length*key2.Length-1))-1];
      
       for (int i = 0; i < (message.Length/(key1.Length*key2.Length-1))-1; i++)
       {
           if (index < message.Length)
           {
               matrix[i] = message.Substring(index,
